...
- a symmetric BLE example
- a one-way BLE example

This manual belongs to the symmetric example in:

- `ble_proximity_peer/ble_proximity_peer.ino`

Use the symmetric pattern when the sensor becomes a stable event state, for example:
...
Use the one-way pattern when the input is brief, noisy, or changes intensity quickly, for example:
...
- stable event states work well when both boards share the same behavior
- fast bursty inputs are often more reliable when one board only sends and the other only responds

The one-way motion sketches in this repository are:
